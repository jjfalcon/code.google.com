...
type
  ...
  TMenuChangeEvent = procedure (Sender: TObject; Source: TMenuItem2; Rebuild: Boolean) of object;
...
{ TMenuItem }
  ...
  private
    ...
    procedure MergeWith(Menu: TMenuItem2);
    procedure RebuildHandle;
    procedure PopulateMenu;
    procedure SubItemChanged(Sender: TObject; Source: TMenuItem2; Rebuild: Boolean);
  ...
  protected
    ...
    function GetItem(Index: Integer): TMenuItem2;
    function GetMenuIndex: Integer;
    function GetAutoHotkeys: Boolean;
    function GetAutoLineReduction: Boolean;
    function InsertNewLine(ABefore: Boolean; AItem: TMenuItem2): Integer;
  ...
  public
    ...
    procedure Insert(Index: Integer; Item: TMenuItem2);
    procedure Delete(Index: Integer);
    procedure Clear;
    procedure Click; virtual;
    function Find(ACaption: string): TMenuItem2;
    function IndexOf(Item: TMenuItem2): Integer;
    ...
    function InsertNewLineBefore(AItem: TMenuItem2): Integer;
    function InsertNewLineAfter(AItem: TMenuItem2): Integer;
    procedure Add(Item: TMenuItem2); overload;
    procedure Add(const AItems: array of TMenuItem2); overload;
    procedure Remove(Item: TMenuItem2);
    ...
    property Items[Index: Integer]: TMenuItem2 read GetItem; default;
    property MenuIndex: Integer read GetMenuIndex write SetMenuIndex;
    property Parent: TMenuItem2 read FParent;
  published
  ...
  protected
    ...
    procedure DoChange(Source: TMenuItem2; Rebuild: Boolean); virtual;
    ...
    procedure Loaded; override;
    procedure MenuChanged(Sender: TObject; Source: TMenuItem2; Rebuild: Boolean); virtual;
  ...
  public
    ...
    function FindItem(Value: Integer; Kind: TFindItemKind): TMenuItem2;
  ...
  published
    property Items: TMenuItem2 read FItems;
  end;
  ...
  protected
    procedure MenuChanged(Sender: TObject; Source: TMenuItem2; Rebuild: Boolean); override;
  ...
  PMenuItem = TMenuItem2;

  TMenuItemStack = class(TStack)
  public
    procedure ClearItem(AItem: TMenuItem2);
  end;
...
function NewMenu(Owner: TComponent; const AName: string; const Items: array of TMenuItem2): TMainMenu;
function NewPopupMenu(Owner: TComponent; const AName: string;
  Alignment: TPopupAlignment; AutoPopup: Boolean; const Items: array of TMenuItem2): TPopupMenu;
function NewSubMenu(const ACaption: string; hCtx: THelpContext;
  const AName: string; const Items: array of TMenuItem2; AEnabled: Boolean = True): TMenuItem2;
function NewItem(const ACaption: string; AShortCut: TShortCut;
  ...
  const AName: string): TMenuItem2;
function NewLine: TMenuItem2;

procedure DrawMenuItem(MenuItem: TMenuItem2; ACanvas: TCanvas; ARect: TRect;
...
procedure IterateMenus(Func: Pointer; Menu1, Menu2: TMenuItem2);
var
  I, J: Integer;
  IIndex, JIndex: Byte;
  ...
  function Iterate(var I: Integer; MenuItem: TMenuItem2; AFunc: Pointer): Boolean;
  var
    Item: TMenuItem2;
  begin
...
constructor TMenuItem2.Create(AOwner: TComponent);
begin
...
procedure TMenuItem2.AppendTo(Menu: HMENU; ARightToLeft: Boolean);
const
...
procedure TMenuItem2.PopulateMenu;
var
  MenuRightToLeft: Boolean;

  function AddIn(MenuItem: TMenuItem2): Boolean;
  begin
...
procedure TMenuItem2.ReadShortCutText(Reader: TReader);
begin
  ShortCut := TextToShortCut(Reader.ReadString);
end;

procedure TMenuItem2.MergeWith(Menu: TMenuItem2);
begin
...
procedure TMenuItem2.RebuildHandle;
var
  I: Integer;
  LRepopulate: Boolean;
begin
...
procedure TMenuItem2.VerifyGroupIndex(Position: Integer; Value: Byte);
var
  I: Integer;
begin
...
function TMenuItem2.GetHandle: HMENU;
begin
  if FHandle = 0 then
  begin
...
procedure TMenuItem2.DefineProperties(Filer: TFiler);
begin
  inherited DefineProperties(Filer);
  Filer.DefineProperty('ShortCutText', ReadShortCutText, nil, False);
end;

procedure TMenuItem2.DoDrawText(ACanvas: TCanvas; const ACaption: string;
  var Rect: TRect; Selected: Boolean; Flags: Longint);
var
  Text: string;
  R: TRect;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Selected and (ColorToRGB($00BCBCBC) = ColorToRGB($002A2A2A)) then
          Font.Color := $00BCBCBC else
          Font.Color := $002A2A2A;
      end;
      DrawText(Handle, PChar(Text), Length(Text), Rect, Flags);
    end;
...
procedure TMenuItem2.DrawItem(ACanvas: TCanvas; ARect: TRect; Selected: Boolean);
begin
  if Assigned(FOnDrawItem) then
    FOnDrawItem(Self, ACanvas, ARect, Selected);
end;

procedure TMenuItem2.AdvancedDrawItem(ACanvas: TCanvas; ARect: TRect;
...
function TMenuItem2.GetImageList: TCustomImageList;
var
  LItem: TMenuItem2;
  LMenu: TMenu;
begin
...
procedure TMenuItem2.MeasureItem(ACanvas: TCanvas; var Width, Height: Integer);
const
  Alignments: array[TPopupAlignment] of Word = (DT_LEFT, DT_RIGHT, DT_CENTER);
var
  Alignment: TPopupAlignment;
...
function TMenuItem2.HasParent: Boolean;
begin
  Result := True;
end;

procedure TMenuItem2.SetBreak(Value: TMenuBreak);
begin
...
procedure TMenuItem2.SetCaption(const Value: string);
begin
...
procedure TMenuItem2.TurnSiblingsOff;
var
  I: Integer;
  Item: TMenuItem2;
begin
...
procedure TMenuItem2.SetChecked(Value: Boolean);
begin
...
procedure TMenuItem2.SetEnabled(Value: Boolean);
begin
...
procedure TMenuItem2.SetGroupIndex(Value: Byte);
begin
...
function TMenuItem2.GetAction: TBasicAction;
begin
...
function TMenuItem2.GetActionLinkClass: TMenuActionLinkClass;
begin
  Result := TMenuActionLink;
end;

function TMenuItem2.GetCount: Integer;
begin
  if FItems = nil then Result := 0
  else Result := FItems.Count;
end;

function TMenuItem2.GetItem(Index: Integer): TMenuItem2;
begin
  if FItems = nil then Error(@SMenuIndexError);
  Result := FItems[Index];
end;

procedure TMenuItem2.SetShortCut(Value: TShortCut);
begin
...
procedure TMenuItem2.SetVisible(Value: Boolean);
begin
...
procedure TMenuItem2.SetImageIndex(Value: TImageIndex);
begin
...
function TMenuItem2.GetMenuIndex: Integer;
begin
  Result := -1;
  if FParent <> nil then Result := FParent.IndexOf(Self);
end;

procedure TMenuItem2.SetMenuIndex(Value: Integer);
var
  Parent: TMenuItem2;
  Count: Integer;
begin
...
procedure TMenuItem2.GetChildren(Proc: TGetChildProc; Root: TComponent);
var
  I: Integer;
begin
...
procedure TMenuItem2.SetChildOrder(Child: TComponent; Order: Integer);
begin
  (Child as TMenuItem2).MenuIndex := Order;
end;

procedure TMenuItem2.SetDefault(Value: Boolean);
var
  I: Integer;
begin
...
procedure TMenuItem2.InitiateAction;
begin
  if FActionLink <> nil then FActionLink.Update;
end;

procedure TMenuItem2.Insert(Index: Integer; Item: TMenuItem2);
begin
  if Item.FParent <> nil then Error(@SMenuReinserted);
  if FItems = nil then FItems := TList.Create;
  if (Index - 1 >= 0) and (Index - 1 < FItems.Count) then
    if Item.GroupIndex < TMenuItem2(FItems[Index - 1]).GroupIndex then
      Item.GroupIndex := TMenuItem2(FItems[Index - 1]).GroupIndex;
...
procedure TMenuItem2.Delete(Index: Integer);
var
  Cur: TMenuItem2;
begin
...
function TMenuItem2.IndexOf(Item: TMenuItem2): Integer;
begin
  Result := -1;
  if FItems <> nil then Result := FItems.IndexOf(Item);
end;

procedure TMenuItem2.Add(Item: TMenuItem2);
begin
  Insert(GetCount, Item);
end;

procedure TMenuItem2.Remove(Item: TMenuItem2);
var
  I: Integer;
begin
  I := IndexOf(Item);
...
procedure TMenuItem2.MenuChanged(Rebuild: Boolean);
var
  Source: TMenuItem2;
begin
...
procedure TMenuItem2.SubItemChanged(Sender: TObject; Source: TMenuItem2; Rebuild: Boolean);
begin
...
function TMenuItem2.GetBitmap: TBitmap;
begin
...
procedure TMenuItem2.SetAction(Value: TBasicAction);
begin
  if Value = nil then
  begin
    FActionLink.Free;
...
procedure TMenuItem2.SetBitmap(Value: TBitmap);
begin
  if FBitmap = nil then FBitmap := TBitmap.Create;
...
procedure TMenuItem2.InitiateActions;
var
  I: Integer;
begin
...
function TMenuItem2.GetParentComponent: TComponent;
begin
...
procedure TMenuItem2.SetParentComponent(Value: TComponent);
begin
  if FParent <> nil then FParent.Remove(Self);
  if Value <> nil then
    if Value is TMenu then
      TMenu(Value).Items.Add(Self)
    else if Value is TMenuItem2 then
      TMenuItem2(Value).Add(Self);
end;

function TMenuItem2.GetParentMenu: TMenu;
var
  MenuItem: TMenuItem2;
begin
  MenuItem := Self;
...
procedure TMenuItem2.SetRadioItem(Value: Boolean);
begin
...
procedure TMenuItem2.ActionChange(Sender: TObject; CheckDefaults: Boolean);
begin
...
procedure TMenuItem2.DoActionChange(Sender: TObject);
begin
  if Sender = Action then ActionChange(Sender, False);
end;

function TMenuItem2.IsCaptionStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsCaptionLinked;
end;

function TMenuItem2.IsCheckedStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsCheckedLinked;
end;

function TMenuItem2.IsEnabledStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsEnabledLinked;
end;

function TMenuItem2.IsHintStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsHintLinked;
end;

function TMenuItem2.IsHelpContextStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsHelpContextLinked;
end;

function TMenuItem2.IsImageIndexStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsImageIndexLinked;
end;

function TMenuItem2.IsShortCutStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsShortCutLinked;
end;

function TMenuItem2.IsVisibleStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsVisibleLinked;
end;

function TMenuItem2.IsOnClickStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsOnExecuteLinked;
end;

procedure TMenuItem2.AssignTo(Dest: TPersistent);
begin
...
procedure TMenuItem2.Notification(AComponent: TComponent;
...
procedure TMenuItem2.SetSubMenuImages(Value: TCustomImageList);
begin
...
procedure TMenuItem2.ImageListChange(Sender: TObject);
begin
  if Sender = SubMenuImages then UpdateItems;
end;

procedure TMenuItem2.UpdateItems;

  function UpdateItem(MenuItem: TMenuItem2): Boolean;
  begin
    Result := False;
...
procedure TMenuItem2.Add(const AItems: array of TMenuItem2);
var
  I: Integer;
begin
...
function TMenuItem2.InternalRethinkHotkeys(ForceRethink: Boolean): Boolean;
var
  ...
  begin
    ...
    for I := 0 to LDoing.Count - 1 do
      LBest.AddObject(TMenuItem2(LDoing.Objects[I]).FCaption, LDoing.Objects[I]);
  ...
  begin
    ...
    try
      ...
      begin
        ...
        TMenuItem2(LBest.Objects[I]).FCaption := LCaption;
      end;
    finally
      LBest.Free;
      LToDo.Free;
...
function TMenuItem2.RethinkHotkeys: Boolean;
begin
...
procedure TMenuItem2.SetAutoHotkeys(const Value: TMenuItemAutoFlag);
begin
...
function TMenuItem2.IsLine: Boolean;
begin
  Result := FCaption = cLineCaption;
end;

function TMenuItem2.Find(ACaption: string): TMenuItem2;
var
  I: Integer;
begin
  Result := nil;
...
function TMenuItem2.InsertNewLine(ABefore: Boolean; AItem: TMenuItem2): Integer;
begin
...
function TMenuItem2.InsertNewLineAfter(AItem: TMenuItem2): Integer;
begin
  Result := InsertNewLine(False, AItem);
end;

function TMenuItem2.InsertNewLineBefore(AItem: TMenuItem2): Integer;
begin
  Result := InsertNewLine(True, AItem);
end;

function TMenuItem2.NewBottomLine: Integer;
begin
  Result := 0;
  if Count = 0 then
    Add(NewLine)
...
function TMenuItem2.NewTopLine: Integer;
begin
  Result := 0;
  if Count = 0 then
    Add(NewLine)
...
function TMenuItem2.InternalRethinkLines(ForceRethink: Boolean): Boolean;
var
  I, LLastAt: Integer;
  LLastBar: TMenuItem2;
begin
...
procedure TMenuItem2.SetAutoLineReduction(const Value: TMenuItemAutoFlag);
begin
...
function TMenuItem2.RethinkLines: Boolean;
begin
...
function TMenuItem2.GetAutoHotkeys: Boolean;
var
...
function TMenuItem2.GetAutoLineReduction: Boolean;
var
  LAuto: TMenuItemAutoFlag;
begin
...
function TMenu.FindItem(Value: Integer; Kind: TFindItemKind): TMenuItem2;
var
  FoundItem: TMenuItem2;

  function Find(Item: TMenuItem2): Boolean;
  var
    I: Integer;
  begin
    Result := False;
...
procedure TMenu.DoChange(Source: TMenuItem2; Rebuild: Boolean);
begin
  if Assigned(FOnChange) then FOnChange(Self, Source, Rebuild);
end;
...
procedure TMenu.MenuChanged(Sender: TObject; Source: TMenuItem2; Rebuild: Boolean);
begin
...
procedure TMainMenu.MenuChanged(Sender: TObject; Source: TMenuItem2; Rebuild: Boolean);
var
  NeedUpdate: Boolean;
begin
  if (FWindowHandle <> 0) then
...
procedure TMenuItemStack.ClearItem(AItem: TMenuItem2);
var
  I: Integer;
begin
...
function NewSubMenu(const ACaption: string; hCtx: THelpContext;
  const AName: string; const Items: array of TMenuItem2; AEnabled: Boolean): TMenuItem2;
var
  I: Integer;
begin
  Result := TMenuItem2.Create(nil);
  for I := Low(Items) to High(Items) do
    Result.Add(Items[I]);
