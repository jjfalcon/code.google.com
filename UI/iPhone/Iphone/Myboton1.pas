...
type
  ...
  TCustomColor = (ccLeather,ccOldGold,ccBuffedGold,ccBottleGreen,ccEmerald,ccGlass,
                  ...
                  ccNone);
  TFreeBoton = class(TGraphicControl)
  private
  // FVariables
     FDrawColor: TColor;
     FLightColor: TColor;
  ...
  // vars
     LightOn: Boolean;
     procedure SpecialEventos(i:integer);
  // constuct
...
procedure Register;
begin
  RegisterComponents('IPhone', [TFreeBoton]);
end;

constructor TFreeBoton.Create(Aowner: TComponent);
begin
    inherited;
     ControlStyle := ControlStyle-[csOpaque,csAcceptsControls];
     Width := 25;
     Height := 25;
     DrawColor := GetColor(clWhite);
     LightColor := GetColor(clWhite);
     BackColor := GetColor(clWhite);
     //Caption := letra;
     FFont := TFont.Create;
     FFont.Color := GetColor(clBlack);
     FFont.Name := 'Verdana';
     FFont.Size := 10;
     FFont.Height:=-20;
     FFont.Style := [];
...
procedure TFreeBoton.setDrawColor(const Value: TColor);
begin
     FDrawColor := GetColor(Value);
     Invalidate;
end;

 procedure TFreeBoton.setLightColor(const Value: TColor);
begin
     FLightColor := GetColor(Value);
     Invalidate;
end;

procedure TFreeBoton.setBackColor(const Value: TColor);
begin
     FBackColor := GetColor(Value);
     Invalidate;
end;

procedure TFreeBoton.setCaption(const Value: String);
begin
     FCaption := Value;
     Invalidate;
end;

procedure TFreeBoton.setFont(const Value: TFont);
begin
     FFont := Value;
     Invalidate;
end;

procedure TFreeBoton.setTextPosition(const Value: TTextPosition);
begin
  FTextPosition := Value;
  Invalidate;
end;

procedure TFreeBoton.setGlyphPosition(const Value: TGlyphPosition);
begin
  FGlyphPosition := Value;
  Invalidate;
end;

procedure TFreeBoton.setGlyph(const Value: TBitmap);
begin
     FGlyph.Assign(Value);
     Invalidate;
end;

procedure TFreeBoton.setTransparentGlyph(const Value: Boolean);
begin
     FTransparentGlyph := Value;
     Invalidate;
end;

procedure TFreeBoton.setDrawDropShadow(const Value: Boolean);
begin
     FDrawDropShadow := Value;
     Invalidate;
end;

procedure TFreeBoton.setDrawLight(const Value: Boolean);
begin
     FDrawLight := Value;
     Invalidate;
end;

procedure TFreeBoton.setLightSize(const Value: Integer);
begin
     FLightSize := Value;
     Invalidate;
end;

procedure TFreeBoton.setCustomColor(const Value: TCustomColor);
begin
     FCustomColor := Value;
     Invalidate;
end;

procedure TFreeBoton.setAutoLightColor(const Value: Boolean);
begin
     FAutoLightColor := Value;
     Invalidate;
end;

procedure TFreeBoton.Paint;
...
procedure TFreeBoton.CMMouseEnter(var msg: TMessage);
begin
     inherited;
     FButtonState := bsOver;
     Repaint;
...
procedure TFreeBoton.CMMouseLeave(var msg: TMessage);
begin
     inherited;
     FButtonState := bsUp;
     Repaint;
...
procedure TFreeBoton.WMLMOUSEDOWN(var msg: TMessage);
begin
     inherited;
     FButtonState := bsDown;
     Repaint;
...
procedure TFreeBoton.WMLMOUSEUP(var msg: TMessage);
begin
     inherited;
     FButtonState := bsOver;
...
function TFreeBoton.GetNewColor(InputColor: TColor; Number: Integer; Additive: Boolean): TColor;
var r,g,b: Integer;
begin
...
function TFreeBoton.RoundNumber(n: Extended): integer;
begin
     Result := Round(Int(n));
end;


function TFreeBoton.GetColor(Color: TColor): TColor;
var NewColor: TColor;
begin
  NewColor := Color;
  case Color of
...
function TFreeBoton.GetTextPosition(CanvasWidth,CanvasHeight:Integer): TPoint;
begin
...
function TFreeBoton.GetGlyphPosition(CanvasWidth,CanvasHeight:Integer): TPoint;
begin
...
function TFreeBoton.GetCustomColor: TColor;
begin
     Case CustomColor of
...
procedure TFreeBoton.SpecialEventos(i:integer);
begin
      FCaption:='';
end;
